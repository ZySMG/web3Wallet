...
// Your existing: Network / Currency / WalletError / EtherscanV2Service
// This service is responsible for: nonce / gasPrice / sendRawTx (three key) + balance (via Etherscan V2)
...
final class EthereumService: EthereumServiceProtocol {
    ...
    // MARK: - Balance (via Etherscan V2, with chainid)
    func getBalance(address: String, currency: Currency, network: Network) -> Observable<Decimal> {
        if let contract = currency.contractAddress, !contract.isEmpty {
            ...
            return etherscan.getTokenBalance(address: address, contractAddress: contract, chainId: network.chainId)
                .map { raw -> Decimal in
                    // raw is minimum unit integer string (e.g. USDC 6 digits)
...
// Small utility: keep 6 decimal places (display friendly)
